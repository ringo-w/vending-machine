...
class VendingMachine {
  ...
  // select chip by ID - check for available stock and matching ID
  selectChip(id) {
    for (let chip of this.inventory.products) {
      if (chip.id === id && chip.currentQty === 0)
        ...
        throw new Error("Incorrect ID. Please try again."); // remove duplicate
      }
    }
  }
  ...
  dispense(id, payment) {
    ...
    for (let chip of this.inventory.products) {
      if (chip.id === id && chip.price > payment) {
        ...
        return this.selectChip(id);
      }
    }
  }

  // return the least amount of change
